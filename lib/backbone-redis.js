...
function errorMessage(err, model, options) {
    ...
    if (!showError) return;
    console.error('Error!', err, "Model:", model, "Options:", options);
    console.trace();
    return this;
};
...
module.exports = {
    ...
    //###process
    // Process incoming messages, delegating them to the correct method,
    ...
    // process : function(socket, data, fn) {
    ...
    //         type   = options.type;
        ...
        // console.log("(process)", type, this.model(type));
    ...
    publish : function(model, options, cb) {
        // console.log("*** PUBLISHING ***", pub, model, options);
        var chan = options.channel;
        if (pub) {
    ...
    create : function(model, options, cb) {
        ...
        db.incr('next.' + type + '.id', function(err, rid) {
            ...
            // console.log("(create model) NEW MODEL ID:", rid);
            model.dateCreated = model.dateModified = new Date().getTime();
            model.id = rid;

            data = JSON.stringify(model);

            db.set(type + ":" + rid, data, function(err, result) {
                if (err) return (errorMessage(err, model, options));
                ...
                // Support for connecting this Object with an user
                if (typeof options.userid !== "undefined") {
                    db.sadd('user:'+options.userid+':'+type, rid,  function(err, result) {
                        if (err) return (errorMessage(err, model, options));
                        if (!result) return (debugMessage('sadd to user', model, options));
                    });
                }
                ...
                if (!options.silent) {
                    // console.log("*** WE ARE NOT SILENT!!!***", model, options);
                    return self.publish(model, options, cb);
                }
                cb && cb(model, options);
                return self;
            });
    ...
    read : function(model, options, cb) {
        ...
        if (model.id) {
            throw "this should not be used:"+model.id;
            db.get(type + ":" + model.id, function(err, result) {
                if (err) return (errorMessage(err, model, options));
                if (!result) return (debugMessage('get', model, options));

                if (typeof result.user !== "undefined" && typeof options.userid !== "undefined") {
                    if (result.user != options.userid) {
                        // Access Denied
                        return (errorMessage("Access denied for userid:"+options.userid, model, options));
                    } else {
                        // Access granted
        ...
        if (typeof options.userid !== "undefined") {
            var skey = "user:"+options.userid+":"+type;
        } else {
            var skey = type;
        }

        // Read all objectsIds from the list and try to fetch each
        db.smembers(skey, function(err, result) {
            if (err) return (errorMessage(err, model, options));
            ...
            // Get all objects by their appropriate name
            var objects = _.map(result, function(it) { return type + ":" + it; });
            db.mget(objects, function(err, result) {
                if (err) return (errorMessage(err, model, options));
                ...
                if (options.process_model) {
                    // console.log("(applying process_model) to", parsed);
                    parsed = _.map(parsed, options.process_model);
                }
    ...
    update : function(model, options, cb) {
        ...
        db.get(type + ":" + id, function(err, result) {
            if (err) return (errorMessage(err, model, options));
            if (!result) return (debugMessage('get', model, options));

            // Check for valid userid when necessary
            // console.log("******* Update result:", result);
            if (typeof result.user !== "undefined" && typeof options.userid !== "undefined" && result.user != options.userid) {
                // Access Denied
                throw "Access denied";
                return (errorMessage("Access denied for userid:"+options.userid, model, options));
            } else {

                // High cohesion, ultra-high coupling...
                if (typeof result.labels == "undefined" || result.labels.length == model.labels.length) {
                ...
                // Access granted     
                db.set(type + ":" + id, data, function(err, result) {
                    if (err) return (errorMessage(err, model, options));
                    if (!result) return (debugMessage('set', model, options));
    ...
    delete : function(model, options, cb) {
        ...
        db.sismember(skey, id, function(err, member) {
            if (err) return (errorMessage(err, model, options));
            if (member) {
                // Remove model from collection set
                db.srem(skey, id, function(err, result) {
                    if (err) return (errorMessage(err, model, options));
                    if (!result) return (debugMessage('srem', model, options));
                });

                // Remove model from global set AND user-linked set when needed
                if (typeof options.userid !== "undefined") {                
                    db.srem(type, id, function(err, result) {
                        if (err) return (errorMessage(err, model, options));
                        if (!result) return (debugMessage('srem2', model, options));
                    });
                }
            }

            db.del(type + ":" + id, function(err, result) {
                if (err) return (errorMessage(err, model, options));
...
_.extend(Backbone.Model.prototype, {
    ...
    publish : function(options, next) {
        // console.log("*** Backbone model push ***");
        var model = this;
        options         || (options = {});
        options.channel || (options.channel = (model.collection) ? _.getUrl(model.collection) : _.getUrl(model));
